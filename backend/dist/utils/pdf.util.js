...
const drawHomeCareLogoFallback = (doc, x, y) => {
    const gray = "#3A3A3A";
    const blue = "#4540E1";
    const fontSize = 28;
...
const generateInvoicePDF = (data, stream) => {
    ...
    if (logoExists === null) {
        logoExists = fs_1.default.existsSync(logoPath);
    }
    // =========================
    // Header
    // =========================
    if (logoExists) {
        ...
        drawHomeCareLogoFallback(doc, 50, 35);
    }
    doc
        .fillColor(darkColor)
        .fontSize(22)
    ...
    if (data.servicePartnerName) {
        doc
            ...
            .font("Helvetica")
            .fontSize(10)
            .text(`Phone: ${data.servicePartnerPhone || "N/A"}`, rightColumnX, detailsY + 42, {
            align: "right",
            width: rightColumnWidth,
        });
    }
    ...
    const serviceNameCenteredY = tableTop + headerHeight + (bodyHeight - serviceNameHeight) / 2;
    doc
        .fillColor(darkColor)
        .font("Helvetica")
        .fontSize(10)
    ...
    doc.font("Helvetica-Bold").text(data.serviceName, x2 + 10, serviceNameCenteredY, {
        width: col2 - 20,
        lineGap: 2,
    });
    doc
    ...
    doc.text(data.paymentMethod || "Online", x4 + 5, centeredY, {
        width: col4 - 10,
        align: "center",
    });
    ...
    doc
        ...
        .text(`${currencySymbol} ${data.subTotal.toFixed(2)}`, splitX + 10, currentY, { width: 75, align: "right" });
    currentY += summaryRowHeight;
    doc
        .text("Tax", summaryLeft + 10, currentY, { width: 100, align: "right" })
    ...
    if (data.discount > 0) {
        ...
        doc
            ...
            .text(`- ${currencySymbol} ${data.discount.toFixed(2)}`, splitX + 10, currentY, { width: 75, align: "right" });
    }
    currentY += summaryRowHeight;
    doc
        ...
        .text(`${currencySymbol} ${data.totalAmount.toFixed(2)}`, splitX + 10, currentY, { width: 75, align: "right" });
    // =========================
    // Footer
    // =========================
    const footerTop = 750;
    ...
    doc
        ...
        .fillColor("#9ca3af")
        .font("Helvetica")
        .text("This is a computer-generated invoice and does not require a signature. For any queries, please contact support@homecare.com", 50, footerTop + 30, { align: "center", width: 500 });
    doc.end();
};
